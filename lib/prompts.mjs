...
import { bytesToSize, convertDuplicates, createSummaryFile, findDivergentDirectories, findDuplicates, findFilesWithString, sanitizeFileName, writeSearchToCSV, writeFilterToCSV, createFilterJsonFile, findFiles } from "./helpers/utils.mjs";
...
const initialAction = () => {
  inquirer
    ...
    .then(answer => {
      ...
      } else if (action === "Find low resolution files") {
        return selectFilterDirectory();
      } else if (action === "Find files with specific tags") {
        return getFinderTags()
...
const getFinderTags = () => {
  return new Promise((resolve, reject) => {
    const script = "defaults read com.apple.finder FavoriteTagNames";
    exec(script, (error, stdout, stderr) => {
      if (error) {
        ...
        const tags = stdout.trim().slice(1, -1).split(",").map(tag => tag.trim().slice(1, -1));
        resolve(tags);
      }
    });
  });
...
const selectFilterDirectory = () => {
  inquirer
    .prompt([
      {
        ...
        message: "What directory would you like to search in?",
        root: rootPath,
        name: "filterDir"
      }
    ])
    .then(async answer => {
      if (answer["filterDir"]) {
        filterDirectory = answer["filterDir"];
        return chooseNewFilterFileLocation("csv");
      } else {
  ...
  inquirer
    ...
    .then(async answer => {
      if (answer["searchDir"] && searchValue) {
        searchDirectory = answer["searchDir"];
        searchResults = await findFilesWithString(answer["searchDir"], searchValue);
        if (searchResults.length > 0) {
          return chooseNewSearchFileLocation("csv");
        } else {
          console.log("No files found. Please try again.");
          return;
        }
      } else {
        console.log("No directory selected. Please try again.");
...
const chooseNewFilterFileLocation = (type) => {
  inquirer
    .prompt([
      {
        type: "file-tree-selection",
        name: "newFilterDirectoryLocation",
        message: "Where do you want to have the .csv file created?",
        onlyShowDir: true,
        enableGoUpperDirectory: true,
        root: cwdPath,
      }
    ...
    .then(answer => {
      return chooseNewFilterFileName(answer["newFilterDirectoryLocation"], type);
    }).catch(error => {
      console.log(error);
    });
};

const chooseNewFilterFileName = (targetPath, fileType) => {
  const sanitizedDirectory = sanitizeFileName(filterDirectory);
  const defaultName = `dirtective-search-low-res-in-${sanitizedDirectory}`;
  inquirer
    .prompt([
      {
        type: "input",
        name: "newFilterFileName",
        message: `What would you like to name the ${fileType} file?`,
        default: defaultName
      }
    ])
    .then(async answer => {
      filterResults = await findFiles(filterDirectory).then((files) => {
        return files;
      }).catch((error) => {
      ...
      if (filterResults.length > 0) {
        await createFilterJsonFile(targetPath, answer["newFilterFileName"], filterResults);
        return await writeFilterToCSV(targetPath, answer["newFilterFileName"], filterResults);
      } else {
        console.log("No files found. Please try again.");
        return;
      }
...
const chooseNewSearchFileLocation = (type) => {
  inquirer
    .prompt([
      {
        type: "file-tree-selection",
        name: "newSearchDirectoryLocation",
        onlyShowDir: true,
        enableGoUpperDirectory: true,
        root: cwdPath,
      }
    ...
    .then(answer => {
      return chooseNewSearchFileName(answer["newSearchDirectoryLocation"], type);
    }).catch(error => {
      console.log(error);
    });
};

const chooseNewSearchFileName = (targetPath, fileType) => {
  const sanitizedDirectory = sanitizeFileName(searchDirectory);
  const defaultName = `dirtective-search-${searchValue}-in-${sanitizedDirectory}`;
  inquirer
    .prompt([
      {
        type: "input",
        name: "newSearchFileName",
        message: `What would you like to name the ${fileType} file?`,
        default: defaultName
      }
    ])
    .then(async answer => {
      await writeSearchToCSV(targetPath, answer["newSearchFileName"], searchResults);
    }).catch(error => {
      console.log(error);
    });
};
  ...
  inquirer
    ...
    .then(async answer => {
      if (!searchValue) {
        ...
        await writeSearchToCSV(targetPath, answer["newFileName"], fileType, searchValue, searchResults);
      }
    }).catch(error => {
      console.log(error);
    });
