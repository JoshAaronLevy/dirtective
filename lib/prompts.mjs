...
import { bytesToSize, convertDuplicates, createSummaryFile, findDivergentDirectories } from "./helpers/utils.mjs";
...
const identifyUniqueFiles = async () => {
  ...
  arr1.forEach(file => {
    if (fileExists(arr2, file.name)) {
      console.log("Found a duplicate: ", file.name);
    } else {
...
const identifyDuplicates = async () => {
  ...
  try {
    const primaryFiles = constants.primaryDirectory.files;
    const secondaryFiles = constants.secondaryDirectory.files;
    const newFiles = [];
    ...
    if (newFiles.length === 0) {
      ...
      totalDuplicateFiles = newFiles.length;
    ...
    .prompt([
      {
        type: "list",
        name: "allDupeAction",
        message: `${totalDuplicateFiles} duplicate files found. What would you like to do?\n`,
        choices: [
...
const chooseNewFileName = (targetPath, fileType) => {
  ...
  inquirer
    ...
    .then(answer => {
      return createSummaryFile(targetPath, answer["newFileName"], fileType, duplicateQueue);
    }).catch(error => {
      console.log(error);
    });
};
